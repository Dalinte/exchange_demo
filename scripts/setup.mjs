...
function syncWebEnvLocal(rootEnv) {
  const webEnvPath = resolve(projectRoot, 'apps/web/.env.local');
  const publicEntries = Object.entries(rootEnv).filter(([k]) => k.startsWith('NEXT_PUBLIC_'));
  if (publicEntries.length === 0) {
    console.warn('No NEXT_PUBLIC_* variables found in .env — skipping apps/web/.env.local');
    return;
  }
  const banner = [
    '# Auto-generated by scripts/setup.mjs from repo-root .env',
    '# Edit .env at the repo root, then re-run `npm run setup`.',
    '',
  ].join('\n');
  const body = publicEntries.map(([k, v]) => `${k}=${v}`).join('\n') + '\n';
  writeFileSync(webEnvPath, banner + body);
  console.log(`Wrote apps/web/.env.local (${publicEntries.length} NEXT_PUBLIC_* vars).`);
}
